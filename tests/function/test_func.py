...
from agent_tools.function.func import FunctionWrapper, WrapperConfig, ArgSchemaParser, CannotParseTypeError, BrokenSchemaError
...
def test_call():
    wrapper = FunctionWrapper(func=mock_function)
    result = wrapper({'a': 'value1', 'b': 2, 'c': 'default'}) # TODO: should work without 'c'
...
def test_function_with_no_return_value():
    ...
    wrapper = FunctionWrapper(func=no_return_func, config=WrapperConfig(parsers=[MockArgSchemaParser]))
...
def test_function_that_raises_exception():
    ...
    wrapper = FunctionWrapper(func=raises_exception, config=WrapperConfig(parsers=[MockArgSchemaParser]))
...
def test_function_that_is_a_generator():
    ...
    wrapper = FunctionWrapper(func=generator_func, config=WrapperConfig(parsers=[MockArgSchemaParser]))
