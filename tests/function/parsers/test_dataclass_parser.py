...
def dataclass_parser():
    return DataclassParser(argtype=DemoClass, rec_parsers=[IntParser, StringParser, BoolParser])
...
def test_raises_error_on_additional_fields(dataclass_parser):
    with pytest.raises(BrokenSchemaError):
        dataclass_parser.parse_value({'id': 1, 'name': 'Test', 'active': True, 'extra': 'field'})
